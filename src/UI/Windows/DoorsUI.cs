...
public class DoorsUI : MonoBehaviour
{
    ...
    private void DoorsWindow(int windowID)
    {
        ...
        foreach (var doorRoom in DoorsHandler.GetRoomsWithDoors())
        {
            ...
            if (GUILayout.Button("关闭", GUIStylePreset.NormalButton, GUILayout.Width(50f)))
            {
            ...
            {
                if (GUILayout.Button("开启", GUIStylePreset.NormalButton, GUILayout.Width(50f)))
                {
            ...
            if (Utils.isHost)
            {
                ...
                spamClose = GUILayout.Toggle(spamClose, "混乱关闭", GUIStylePreset.NormalToggle);
                //垃圾邮件关闭？
                ...
                if (map is MapNames.Polus or MapNames.Airship or MapNames.Fungle)
                {
                    ...
                    spamOpen = GUILayout.Toggle(spamOpen, "混乱打开", GUIStylePreset.NormalToggle);
                    //垃圾邮件打开？
        ...
        if (GUILayout.Button("关闭所有", GUIStylePreset.NormalButton))
        {
        ...
        if (map is MapNames.Polus or MapNames.Airship or MapNames.Fungle)
        {
            if (GUILayout.Button("打开所有", GUIStylePreset.NormalButton))
            {
        ...
        if (Utils.isHost)
        {
            CheatToggles.spamCloseAllDoors = GUILayout.Toggle(CheatToggles.spamCloseAllDoors, "混乱关闭所有", GUIStylePreset.NormalToggle);
            //垃圾邮件关闭所有？

            if (map is MapNames.Polus or MapNames.Airship or MapNames.Fungle)
            {
                CheatToggles.spamOpenAllDoors = GUILayout.Toggle(CheatToggles.spamOpenAllDoors, "混乱打开所有", GUIStylePreset.NormalToggle);
                //垃圾邮件打开所有
            }
        }
        else
